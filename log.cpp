...
void CLogFile::printf(_In_ PCSTR format, ...)
{
	...
	va_start(ap, format);
	char buf[0x800];
	int _len = sprintf_s(buf, _countof(buf), "[%02u:%02u:%02u] ", tf.Hour, tf.Minute, tf.Second), len;

	if (0 < (len = _vsnprintf_s(buf + _len, _countof(buf) - _len, _TRUNCATE, format, ap)))
	{
		HANDLE hFile;
		...
		if (hFile = _hFile) WriteFile(hFile, buf, _len + len, &ft.dwLowDateTime, 0);
